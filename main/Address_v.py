    ...
    if request.method in ["POST", "GET"]:
        ...
        if  __sfsh__=='yes':
            if datas[0].get('sfsh')=='no':
                msg['code']=other_code
                msg['msg'] = "The account has been locked. Please contact the administrator for verification!"
                return JsonResponse(msg)
...
def address_list(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        #获取全部列名
        columns=  address.getallcolumn( address, address)
        #表属性[foreEndList]前台list:和后台默认的list列表页相似,只是摆在前台,否:指没有此页,是:表示有此页(不需要登陆即可查看),Before boarding:表示有此页且需要登陆后才能查看
        try:
...
def address_save(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        for i in address_ids:
            error = address.updatebyparams(address,address, {'id':i,"isdefault":"no"})
        return JsonResponse(msg)
...
def address_info(request,id_):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if __browseClick__=="yes"  and  "clicknum"  in address.getallcolumn(address,address):
            try:
...
def address_alipay(request):
    if request.method in ["POST", "GET"]:
        ...
        order_string = alipay.api_alipay_trade_page_pay(
            ...
            return_url='http://localhost:8080/django4j60g/address/notify',
            #notify_url=''
        )
...
def address_notify(request):
    if request.method in ["POST", "GET"]:
        req_dict = request.session.get("req_dict")
        out_trade_no = req_dict['out_trade_no']
        cursor = connection.cursor()
        
        return redirect('http://localhost:8080/django4j60g/admin/dist/index.html#/address')
