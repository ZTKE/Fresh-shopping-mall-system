...
def orders_info(request,id_):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if __browseClick__=="yes"  and  "clicknum"  in orders.getallcolumn(orders,orders):
            try:
...
def orders_o_value(request, xColumnName, yColumnName):
    if request.method in ["POST", "GET"]:
        ...
        sql = "SELECT {0}, sum({1}) AS total FROM orders {2} GROUP BY {0}".format(xColumnName, yColumnName, where)
        L = []
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
...
def orders_alipay(request):
    if request.method in ["POST", "GET"]:
        ...
        order_string = alipay.api_alipay_trade_page_pay(
            ...
            return_url='http://localhost:8080/django4j60g/orders/notify',
            #notify_url=''
        )
...
def orders_notify(request):
    if request.method in ["POST", "GET"]:
        req_dict = request.session.get("req_dict")
        out_trade_no = req_dict['out_trade_no']
        cursor = connection.cursor()
        cursor.execute("update orders set status='paid' where orderid = '{0}'".format(out_trade_no))
        
        return redirect('http://localhost:8080/django4j60g/admin/dist/index.html#/orders')
